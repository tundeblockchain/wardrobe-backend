...
fixes/gemini-adopt-interior: classify and colour adopt Interior-design-backend's working Gemini request (`v1beta` `:generateContent` + `?key=` query, `content-type` only — no `x-goog-api-key`). Model stays `gemini-2.5-flash-lite` and is not remapped to `gemini-2.5-flash`. Jira key will be added. A remaining non-404 failure is still terminal `FAILED` + `processingError`.
