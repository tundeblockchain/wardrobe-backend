...
/**
 * Standard Webhooks / Svix signature scheme used by Resend and Superwall:
 * https://docs.svix.com/receiving/verifying-payloads/how-manual
 *
 * Headers: svix-id, svix-timestamp, svix-signature
 * Secret: whsec_<base64>
 * Signed content: `${id}.${timestamp}.${rawBody}`
 * Signature: HMAC-SHA256, base64, space-separated `v1,<sig>` tokens
 * Replay window: 5 minutes (Svix default)
 */
export const SVIX_WEBHOOK_TOLERANCE_SECONDS = 300;
...
export function verifySvixWebhookSignature(options: {
  payload: string;
  ...
  const tolerance = options.toleranceSeconds ?? SVIX_WEBHOOK_TOLERANCE_SECONDS;
...
export function signSvixWebhook(options: {
  payload: string;
  id: string;
  timestamp: string;
  webhookSecret: string;
